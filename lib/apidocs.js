...
function apijsonRoute() {
     ...
     * @returns
     */
    apijson.get('/api.json:lang', function(req, res) {
        getMasterJSON();
    ...
    function handle(tenant) {
        console.log(new Date(), 'Starting to obtain active docs for tenant' + tenant.name);
    ...
    function addDoc(api, tenant , lang ,doc, apis) {
        ...
        apiToAdd.humanUrl = 'https://' + tenant.admin_domain
        var x = 'https://' + tenant.name +'.api.canada.ca/' + lang +'/products/detail?api='+ api.service.system_name
        apis.push(apiToAdd);
