...
function apijsonRoute() {
          ...
          docsPromise.then(function (payload)
              {
                ...
                console.log(new Date(), 'These are the active docs belonging to the tenant ' + payload );
  ...
  function processSingleTenant(item, activeDocsBelongingToTenant) {
    ...
    tenant.apis = [];
    console.log(new Date(), 'Getting the API(s) belonging to tenant');
    var apiPromise = getAPISByTenant(item.id);
    apiPromise.then(function(payload) {
      console.log(new Date(), 'These are the api(s) belonging to the tenant' + payload);
    ...
    return new Promise(function(resolve, reject) {
      ...
      request(url, function(err, response, body) {
        ...
        if (err)
          return reject(err);
        try {
          console.log("cleaning up docs");
          var str = body;
          str.replace(/\\/g, "");
          console.log("docs cleaned up " + JSON.parse(str));
          resolve(JSON.parse(JSON.parse(str)));
          console.log(new Date(), 'Active docs fo tenant returned ' + tid);
        } catch (e) {
          reject(e);
        }
      });
