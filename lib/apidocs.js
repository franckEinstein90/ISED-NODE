...
function apijsonRoute() {
    ...
    apijson.get('/api.json', function(req, res) {
        ...
        var tnts = [];
        processTenants(validTenants, tnts );
        console.log(new Date(), 'This is the size of tenants' +  tnts.length);
        res.json(tnts);
    });
     ...
     */
    async function processTenants(validTenants, tnts ) {
        for (const tenant of validTenants) {
          const tenantPopulated =  await handle(tenant);
          pushToMasterTenant( tnts,tenantPopulated);
        }
    ...
    function handle(tenant) {
        ...
        return docsPromise.then(function(payload) {
     ...
     */
    function processSingleTenant(item, activeDocsBelongingToTenant,tnts) {
        var tenant = {};
        ...
        return apiPromise.then(function(payload) {
    ...
    function pushToMasterTenant(arr, obj) {
        ...
        if (index === -1) {
           console.log("pushing the following tenant " + obj.name);
            arr.push(obj);
            console.log("the size of tenants 2 is " + arr.length);
        } else {
          console.log("the size of tenants 3 is " + arr.length);
            arr[index] = obj;
        }
    }
