...
function apijsonRoute() {
  ...
  apijson.get('/', function(req, res) {
    console.log(new Date(), 'In apijsonRoute  get  req.query=', req.query);
  ...
  apijson.get('/api.json', function(req, res)
  {
    ...
    processTenants(validTenants, tenants);  
    res.json(tenants);
  ...
  /**
   * asynch parent func that starts the seq..
   */
  async function processTenants(validTenants, tenants)
  {
    for(const tenant of validTenants)
      {
        await handle(tenant, tenants);
      }
  }
   ...
   */
  function handle(tenant, tenants)
      {
          ...
          docsPromise.then(function (payload)
              {
                ...
                processSingleTenant(tenant,activeDocsBelongingToTenant, tenants);
              }
          );
      };
   ...
   */
  function processSingleTenant(item, activeDocsBelongingToTenant, tenants) {
    var tenant = {};
...
/**
 * Check if formal active docs that contain swagger content exist for a given API
 */
