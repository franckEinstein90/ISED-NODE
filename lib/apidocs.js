    ...
    function processSingleTenant(item, activeDocsBelongingToTenant) {
        ...
        apiPromise.then(function(payload) {
            ...
            apisBelongingToTenant.forEach(function(api) {
                ...
                var doc_localized = getAssociatedDoc(api.service.system_name + '-' + lang, activeDocsBelongingToTenant,api,item);
                
                doc_localized.then(function(doc) {
                    if (doc) {
                        var n = new Date()
                        console.log('Adding doc to tenant ' + api.service.system_name +' ' + n.getHours() + ":" + n.getMinutes() + ":" + n.getSeconds());
                       
                        addDoc(api,tenant,lang,doc, tenant.apis);
                    }
