    ...
    function processSingleTenant(item, activeDocsBelongingToTenant) {
        ...
        apiPromise.then(function(payload) {
            ...
            apisBelongingToTenant.forEach(function(api) {
                ...
                if (doc_fr) {
                    addDoc(doc_fr, tenant.apis);
                }
                var doc_en = getAssociatedDoc(api.service.system_name + "-en", activeDocsBelongingToTenant);
                if (doc_en) {
                    addDoc(doc_en, tenant.apis);
                }
            });
