...
function apijsonRoute() {
  ...
  function handle(tenant)
      {
          ...
          docsPromise.then(function (payload)
              {
                var activeDocsBelongingToTenant = JSON.parse(JSON.stringify(payload));
  ...
  function processSingleTenant(item, activeDocsBelongingToTenant) {
    ...
    apiPromise.then(function(payload) {
      ...
      console.log("this is  the api(s) payload " + payload);
      var apisBelongingToTenant =   JSON.parse(JSON.stringify(payload));
      
      if(apisBelongingToTenant === Array)
        {
        console.log("the apis are now array");
        }
      else
        {
        console.log("the apis are not array");
        }
      console.log(new Date(), 'These are the api(s) belonging to the tenant ' + item.name + "api data: " + payload);
