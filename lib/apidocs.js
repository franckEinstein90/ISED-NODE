  ...
  function processSingleTenant(item, activeDocsBelongingToTenant) {
    ...
    apiPromise.then(function(payload) {
      ...
      apisBelongingToTenant.forEach(function(api){
        console.log(new Date(), 'iterating through api ' + api.service.system_name  );
        activeDocsBelongingToTenant.forEach(function(doc, api)
        {
          console.log(new Date(), 'iterating through api again  ' + api.service.system_name  );
            if( doc.api_doc.id === api.service.system_name +"-fr" || doc.system_name === api.system_name +"-en")
             {
               var apiToAdd = new Object();
               apiToAdd.name = doc.api_doc.body.info.title;
               apiToAdd.description = doc.api_doc.body.info.description;
               pushToArray(tenant.apis, apiToAdd);
             }
        })
      });
    })
