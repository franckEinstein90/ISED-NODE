...
function apijsonRoute() {
    ...
    apijson.get('/apitest.json', function(req, res) {
        ...
        if (!email) {
            ...
            async.eachSeries(validTenants, function(tenant, callback) {
        	    ...
        	    async.waterfall([ async.apply(getApis,tenant), async.apply(addAPIDocs,tenant)],
	...
	return new Promise(function (resolve, reject) {
	        ...
	        let validateAPI  = function (api) {
	            const url = 'https://' + tenant.admin_domain + '/admin/api/services/' + api.service.id + '/features.json?access_token=' + tenant.access_token;
	               ...
	               rp.get(url, {json: true})
	                    .then(function (response) {
	                	      ...
	                	      var add = true;
	                              ...
	                              if (add) {
	                                  console.log("pushing to eligible " + api.api.service.system_name );
	                                  apis.push(api);
    ...
    const  addAPIDocs = function (tenant,cb) {
        ...
        let getActiveDocsForTenant = function (tenant) {
            const url =  'https://' + tenant.admin_domain + '/admin/api/active_docs.json?access_token=' + tenant.access_token;
