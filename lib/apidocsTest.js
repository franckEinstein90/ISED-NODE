...
function apijsonRoute() {
    ...
    /**
     * Get all the Active Docs associated with the tenant
     * TODO: RENAME this to be addActiveDocs
     */
	...
	let getPlansByUser = function(tenant){
	    ...
	    var tenantWithResource = JSONALL.master.tenants.filter(function(el) {
            ...
            tenantWithResource = tenantWithResource[0];
            const url = 'https://' + tenantWithResource.admin_domain + '/admin/api/account_plans/find.json?access_token=' + tenantWithResource.access_token +'&email=' + encodeURIComponent(tenant.authenticatedUser);
            ...
            return new Promise(function(resolve, reject) {
                rp.get(url, {
                    ...
                    .then(function(response) {
                        tenant.plansforUser = response.plans;
                        resolve();
                    }).catch(function (err) {
                	reject();
                    })
    ...
    /**
     * Do a final validation to check if an API is eligible for display on the portal
     */
