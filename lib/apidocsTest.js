    ...
    const getPlans = function(tenant, cb) {
        ...
        let getPlansByUser = function(tenant) {
            ...
            return new Promise(function(resolve, reject) {
                rp.get(url)
                    .then(function(response) {
                	console.log("This is the xmlresp " + response) ;
                	var jsonresp = xmlParser.toJson(response);
                	console.log("This is the json resp " + jsonresp);
                        tenant.plansforUser = JSON.parse(jsonresp).account.plans;
                        resolve();
                    }).catch(function(err) {
                        reject(err);
                    })
    ...
    const validateUserByPlans = function(tenant, cb) {
        ...
        return new Promise(function(resolve, reject) {
            let geUserAccessByPlans = function(plan) {
                ...
                tenantWithResource = tenantWithResource[0];
                const url = 'https://' + tenantWithResource.admin_domain + '/admin/api/account_plans/' + plan.plan.id + '/features.json?access_token=' + tenantWithResource.access_token;
