...
function apijsonRoute() {
                ...
                docsPromise.then(function(payload) {
                    ...
                    apiPromise.then(function(payload) {
                        ...
                        apisBelongingToTenant.forEach(function(api) {
                            var doc_localized = getAssociatedDoc(api.service.system_name + '-' + lang, activeDocsBelongingToTenant,api,tenant);
                            
                            doc_localized.then(function(doc){
                        	
                        	 if (doc) {
                          	   console.log("found a localized doc");
                          	addDocOrig(api, tenantToAdd, lang, doc_localized)
                              }
                            });
                            
                           
                        });
                        console.log("adding tenants to master");
     ...
     * Add the swagger content to the api
     */
    function addDoc(api, tenantToAdd, lang, doc, tenant, callback) {
        console.log("mar ja sale");
