...
function apijsonRoute() {
	...
	async.each(validTenants, function(tenant, callback) {
		...
		tenantToAdd.maintainers.fn = "Don Vo";
		tenantToAdd.maintainers.email = "don.vo@canada.ca";
		tenantToAdd.maintainers.url = "https://api.canada.ca";
		tenantToAdd.apis = [];
		console.log('getting apis for ' + tenant.name );
		...
		apiPromise.then(function(payload) {
			...
			console.log('getting docs for ' + api.service.system_name );
		    ...
		    console.log('pusing ' + tenantToAdd.name );
	    ...
	    // if any of the tenant additions lead to an error....not
	    // happening
	    // in our case but
	    if (err) {
     ...
     */
    function handle(tenant, tenants,callback) {
	var docsPromise = getActiveDocsForTenant(tenant.id,lang);
	docsPromise.then(function(payload) {
	    var activeDocsBelongingToTenant = JSON.parse(payload).api_docs;

	    processSingleTenant(tenant, activeDocsBelongingToTenant, tenants,callback);
	},function(err){ callback(null);});
    };
     ...
     */
    function processSingleTenant(item, activeDocsBelongingToTenant,tenants,callback) {
	var tenant = {};
