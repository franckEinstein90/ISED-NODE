	...
	if (!email) {
		...
		docsPromise.then(function(payload) {
			...
			apisBelongingToTenant.forEach(function(api) {
        				...
        				if (doc_localized) {
        				    handlePublicApi(api, tenantToAdd, lang, doc_localized);
        				}
			    });
	    ...
	    function handlePublicApi(api, tenant,lang, doc_localized)
	    {
		...
		visibilityPromise.then(function(payload) {
		    ...
		    if(add)
		    {
			 addDoc(api,tenant,lang,doc_localized)
		    }
	     ...
	     */
	    function addDoc(api, tenant, lang, doc) {
