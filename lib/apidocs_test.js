...
function apijsonRoute() {
	...
	if (!email) {
		...
		docsPromise.then(function(payload) {
			...
			apisBelongingToTenant.forEach(function(api) {
        				...
        				if (doc_localized) {
        				    addDoc(api,tenantToAdd,lang,doc_localized)
        				  //  handlePublicApi(api, tenant, lang, doc_localized, tenantToAdd,callback);
        				}
			    });
	...
	return new Promise(function(resolve, reject) {
		...
		if (err)
		    console.log("getting features err" + err);
		    return reject(err);
		try {
		    console.log("resolving api visib" + body);
		    resolve(body);
		} catch (e) {
		    console.log("getting features err" + e);
		    reject(e);
		}
	    });
	});
	    ...
	    function handlePublicApi(api, tenant,lang, doc_localized,tenantToAdd,callback)
	    {
		
		var visibilityPromise = getAPIVisibility(api.service.id, tenant);
		visibilityPromise.then(function(payload) {
		    console.log("resolving api visib" + payload);
		    var add =  true;
		    var featureData = JSON.parse(payload);
		    console.log("logging feature data");
		    console.log(featureData);
		    if(featureData.features.length != 0)
		    {
			featureData.features.forEach(function(element) {
			    console.log("This is the feature system name " + element.feature.system_name );
		...
		function(err) {
		    if (err) {
			    // One of the iterations produced an error.
			    // All processing will now stop.
			    console.log('Failed to get visib' + err);
			} 
		    else
			{
			    console.log('got the visib');
			}
	        });
	     ...
	     */
	    function addDoc(api, tenantToAdd, lang, doc) {
		
		var visibilityPromise = getAPIVisibility(api.service.id, tenantToAdd);
		visibilityPromise.then(function(payload) {
		    console.log("resolving api visib" + payload);
		    var add =  true;
		    var featureData = JSON.parse(payload);
