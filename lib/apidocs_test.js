	...
	if (!email) {
		...
		docsPromise.then(function(payload) {
			...
			apisBelongingToTenant.forEach(function(api) {
        				...
        				if (doc_localized) {
        				    handlePublicApi(api, tenant, lang, doc_localized, tenantToAdd);
        				}
			    });
	    ...
	    function handlePublicApi(api, tenant,lang, doc_localized,tenantToAdd)
	    {
		 console.log("This is the service " + api.service.id  );
		...
		console.log("This is the id of the api " + id + " this is the tenant " + tenant.access_token);
