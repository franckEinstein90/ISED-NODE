        ...
        if (!email) {
            ...
            async.each(validTenants, function(tenant, callback) {
                    ...
                    getApisChained(tenant.id).then(function (servicesTest) {
                	  console.log('Got the following api:'+ servicesTest[0].service.name )
                    });
    ...
    function getApisChained(tid) {
	 var tenant = JSONALL.master.tenants.filter(function(el) {
	  ...
	  .then(function(response) {
	      ...
	      var url2 = 'https://' + tenant.admin_domain + '/admin/api/services/' + api.service.id + '/features.json?access_token=' + tenant.access_token;
