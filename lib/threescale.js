...
function threescaleRoute() {
  ...
  threescale.get('/api.json', function(req, res)
  {
    // get all tenants
    // get all services
    // get all activelist docs
    // for each service... from activelist docs get an entry that is system_name _en and system_name_fr
    // then do the mapping... 
...
// RETURN TENTANTS AND SERVICE API LIST FROM FILE(DB)
  threescale.get('/apis', function(req, res) {
    // READ THE MASTER FILE
    getMasterJSON();
    // FILTER BASED ON VISIBLE ATTRIBUTE ==> TRUE
    var filteredArray = JSONALL.master.tenants.filter(function(el) {
  ...
  async function processAPIs(filteredArray)
  {
    for(const item of filteredArray)
      {
        console.log("bhai api add kar rahe hai for this guy " + item.name);
   ...
   * 
   */
  function updateAPI(item) {
    
    
    var promise = getAPISByTenant(item.id);
    promise.then(function(payload) {
      // iterate over payload of api(s) and add 
      compositeAPIList.push(payload);
    })
  }
  ...
  function updateTenant(item) {
    ...
    promise.then(function(payload) {
      item.apis = payload;
      pushToArray(tenanListDynamic,item);
      return item;
    })
  }
