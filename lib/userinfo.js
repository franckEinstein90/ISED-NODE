...
function userinfoRoute() {
	...
	processTenants(validTenants,email,subscriptions).then(
		() => {   
    ...
    async function processTenants(validTenants,email,subscriptions) {
	for (const tenant of validTenants) {
	    await handle(tenant,email,subscriptions);
	}
    }


    function handle(tenant,email,subscriptions)
    {
