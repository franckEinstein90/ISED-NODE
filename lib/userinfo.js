...
function  userinfoRoute() {
                ...
                if (JSON.parse(payLoad).status) {
                	...
                	for (var i = 0; i < arrayLength; i++) {
                	    var newLink = {};
                            ...
                            {
                        	 newLink.href = "https://" + tenant.name + ".beta.api.canada.ca/admin/applications/" +  apps.applications[i].application.id + "?lang="+lang;
                            }
                    	    ...
                    	    {
                    		// call the other e.p and that will populate the cache
				 var servicePromise = getTenantSubscriptionKeysForUser(apps.applications[i].application.service_id,tenant);
				 servicePromise.then(function(serviceResponse) {
                                    if (JSON.parse(serviceResponse).status) {
                                       apps.applications[i].application.apiname = apps.applications[i].application.service_id;
                                    } else {
			                apps.applications[i].application.apiname = JSON.parse(serviceResponse).service.name;
					cache.put(tenant.name +'-'+ apps.applications[i].application.service_id +'-'+ lang,JSON.parse(serviceResponse).service.name,1500 * 60000);
				    }
				 });
                    	    }
                	 }
     ...
     * Get the subscription info for the user for a given tenant
     */
    function getServiceNameForId(id, tenant) {
        console.log("this is the service id " + id);
        return new Promise(function(resolve, reject) {
            var url = 'https://' + tenant.admin_domain + '/admin/api/services/' + id + '.json?access_token=' + tenant.access_token;
            console.log("this is the url" + url);
            request(url, function(err, response, body) {
                if (err)
                    return reject(err);
