...
function userinfoRoute() {
    ...
    userinfo.get('/userinfo.json', function(req, res) {
        ...
        for (const tenant of validTenants) {
         ...
         /*
	     * if(keys) { var tenantToAdd = {}; tenantToAdd.id = tenant.id;
	     * tenantToAdd.keys = keys; tenants.push(tenant); } }
	     * subscriptions.tenants = tenants; res.json(subscriptions);
	     */
    }});
	...
	// "https://ised-isde-admin.beta.api.canada.ca/admin/api/accounts/find.xml?access_token=d4c76697771b79b89033c287c546bdc3e5a73fb701b05a4d287579cac173f9ac&email=dontvo%2Bsso%40gmail.com"
	  ...
	  console.log("sire this is the url " + url);
	  ...
	  request(url, function(err, response, body) {
	      if (err)
	      {
		      return console.error('query failed:', error);
	      }
		    console.log('query!  Server responded with:', body);
	     
		    // var userid = JSON.parse(body).account.id;
		   ...
		   // {"account":{"id":9,"created_at":"2019-01-30T11:00:01-05:00","updated_at":"2019-01-30T11:00:40-05:00",
		    //"credit_card_stored":false,"monthly_billing_enabled":true,"monthly_charging_enabled":true,"state":"approved",
		    //"links":[{"rel":"self","href":"https://ised-isde-admin.beta.api.canada.ca/admin/api/accounts/9"},
		    //{"rel":"users","href":"https://ised-isde-admin.beta.api.canada.ca/admin/api/accounts/9/users"}],"org_name":"ISED "}}
